const Discord = require("discord.js");

module.exports = {
    name: "help",
    execute(message, args, bot, prefix) {
        let embed = new Discord.MessageEmbed()
        .setTitle("Liste de mes commandes")
        .setThumbnail(bot.user.displayAvatarURL())
        .setColor('#bf9322')
        .addField("Générales", `**${prefix}help** Affiche ce message ! :tada:\n**${prefix}botinfo** Affiche des informations concernant le bot:\n**${prefix}invite** Invitez moi !`)
        .addField("Blog", `**${prefix}blog** Affichez dynamiquement les 5 derniers blogs officiels de Fortnite`, true)
        .addField("Info", `**${prefix}info** Permet de rechercher des cosmétiques du Battle Royale`)
        .addField("Map", `**${prefix}map** Montre la map du Battle Royale`, true)
        .addField("News", `**${prefix}news** Donne les actualités en jeux (\`br\`, \`creatif\` ou \`stw\`)`, true)
        .addField("Shop", `**${prefix}shop** Affiche le shop du Battle Royale`, true)
        .addField("Stats", `**${prefix}stats** Affiche le menu de stats`, true)
        .addField("Status", `**${prefix}status** Récupère le status des serveurs`, true)
        .addField("Clé AES", `**${prefix}aes** Récupérez la clé principale et les clés des fichiers PAK dynamiques (leaker)`, true)
        .addField("Leak", `**${prefix}leak** Affiche les cosmétiques disponibles dans les fichiers du jeu depuis la dernière mise à jour`, true)
        .setFooter(message.author.username, message.author.displayAvatarURL({dynamic: true}))
        .setTimestamp()
        message.channel.send(embed)
    }
}
